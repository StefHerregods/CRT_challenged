################################################################################
# Collapsing confidence bounds analysis on Bang et al. (2019) data using the   #
# Herregods et al. (2023) model                                                #
################################################################################
...
df <- read.csv('data/observations/data_Bang_2019_Exp2.csv')
df_recoded <- df %>% 
  mutate(sub = Subj_idx,
         cj = Confidence,
         rt = RT_dec,
