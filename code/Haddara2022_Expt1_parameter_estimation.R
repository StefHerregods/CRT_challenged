...
df <- read.csv('data/observations/data_Haddara_2022_Expt1.csv')
df_recoded <- df %>% 
  mutate(sub = Subj_idx,
         cj = Confidence,
         rt = RT_dec,
  ...
  select(sub, cj, rt, rtconf, manipulation, cor)
...
write.csv(df_preprocessed, "data/observations/data_Haddara_2022_Expt1_preprocessed.csv", row.names = F)
...
for(i in 1:N_subs){  # For each participant separately
  ...
  for(c in 1:N_experiments){  # For each condition separately 
    ...
    file_name <- paste0('data/model_parameters/Haddara2022_Expt1/Haddara2022_Expt1_results_sub_', subs[i], '_experiment', experiment_labels[c], '.Rdata')
...
write.csv(df_params, file = 'data/model_parameters/Haddara2022_Expt1/Haddara2022_Expt1_params.csv', row.names = F)
...
write.csv(df_predictions, file = 'data/model_predictions/Haddara2022_Expt1_predictions.csv', row.names = F)
